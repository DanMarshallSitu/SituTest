...
using SituSystems.SituTest.Services.AzureServiceBusApi;
...
namespace SituSystems.SituTest.Services
{
    public class ArtifactStoreService : IArtifactStoreService
    {
        ...
        private readonly string _situTestVersion;
        private readonly string _situTestEnvironment;
            ...
            IOptions<AzureServiceBusApi.ServiceBusSettings> serviceBusSettings)
        {
            ...
            _situTestVersion = configuration.GetValue<string>("AppVersion");
            _situTestEnvironment = configuration.GetValue<string>("Environment");
        ...
        private async Task SendRendererHealthCheck()
        {
            try
            {
                ...
                var metaData = new Dictionary<string, string>
                {
                    ...
                    {"appVersion", _situTestVersion},
                    {"appEnvironment", _situTestEnvironment},
                    {"processingQueues", ""},
                    {"gpuNames", gpuNames }
                };
                if (_processRecipeRequestMessage != null)
        ...
        private async Task<RendererConfig> GetRendererConfig()
        {
            ...
            {
                Log.Error("SituTest could not retrieve config! {Errors}", string.Join(", ", getRendererConfigResponse.ErrorMessages));
                throw new Exception($"SituTest could not retrieve config! {string.Join(", ", getRendererConfigResponse.ErrorMessages)}");
            }
